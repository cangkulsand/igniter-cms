...
- **Phase 2 (refactor):** 🟢 #1 DONE (Extract Class) — 7 controllers in `app/Controllers/Admin/`, `AdminController` now `index()` only, 27 routes repointed (URLs identical, verified via normalized routes diff + reflection + 302 smoke test). **Not yet committed.** Remaining order: #5→#4→#3, one labelled commit each.
